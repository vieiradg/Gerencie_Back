...
def get_property_details(user_data, property_id):
    ...
    try:
        ...
        return jsonify({"message": "Erro ao buscar detalhes do imóvel", "error": str(e)}), 500
...
def update_property(user_data, property_id):
    ...
    try:
        property_to_update = propertyModel.query.filter_by(id=property_id, user_id=user_id).first()

        if not property_to_update:
            return jsonify({"message": "Imóvel não encontrado ou não autorizado"}), 404

        property_to_update.house_street = data.get("house_street", property_to_update.house_street)
        property_to_update.house_number = data.get("house_number", property_to_update.house_number)
        property_to_update.house_complement = data.get("house_complement", property_to_update.house_complement)
        property_to_update.city = data.get("city", property_to_update.city)
        property_to_update.house_neighborhood = data.get("house_neighborhood", property_to_update.house_neighborhood)
        property_to_update.postal_code = data.get("postal_code", property_to_update.postal_code)

        db.session.commit()

        return jsonify({"message": "Imóvel atualizado com sucesso", "property": property_to_update.to_dict()}), 200
...
def delete_property(user_data, property_id):
    ...
    try:
        property_to_delete = propertyModel.query.filter_by(id=property_id, user_id=user_id).first()
